...
class TestScannerPath:
    """Scanner 投递时填 path 字段."""
    ...
    def env(self, tmp_path):
        ...
        return {"scanner": scanner, "data_dir": data_dir, "ch": ch}

    @pytest.mark.asyncio
    async def test_mention_投递_path_email(self, env):
        """mention 投递 path=email (必答)."""
        env["ch"].append(
            from_="alice", content="@bot hi", type="mention", mentions=["bot"],
        )
        await env["scanner"]._scan_once()
        from agents_chat.v2.files.mailbox import Mailbox
        mb = Mailbox(env["data_dir"] / "mailboxes" / "bot.json", "bot")
        mails = mb.peek()
        assert len(mails) == 1
        assert mails[0].get("path") == "email"

    @pytest.mark.asyncio
    async def test_broadcast_投递_path_broadcast(self, env):
        """[TASK] 广播投递 path=broadcast (可 skip)."""
        env["ch"].append(
            from_="alice", content="[TASK] do something", type="task_broadcast",
        )
        await env["scanner"]._scan_once()
        from agents_chat.v2.files.mailbox import Mailbox
        mb = Mailbox(env["data_dir"] / "mailboxes" / "bot.json", "bot")
        mails = mb.peek()
        assert len(mails) == 1
        assert mails[0].get("path") == "broadcast"
