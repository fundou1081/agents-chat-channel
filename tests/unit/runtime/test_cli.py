...
from agents_chat.infra.cli import CLIResponse, new_session_id
...
class TestMockCLI:
    ...
    async def test_first_invoke_creates_session(self):
        cli = MockCLI()
        r = await cli.execute("hello world")
        assert r.ok
    ...
    async def test_resume_reuses_session(self):
        cli = MockCLI()
        r1 = await cli.execute("first task")
        sid = r1.new_session_id
        r2 = await cli.execute("continue", session_id=sid)
    ...
    async def test_extract_task_id_from_prompt(self):
        cli = MockCLI()
        r = await cli.execute("process task_abc123 now")
    ...
    async def test_call_count(self):
        ...
        await cli.execute("x")
        await cli.execute("y")
        await cli.execute("z", session_id="mock_1")
        assert cli.call_count == 3

    @pytest.mark.asyncio
    async def test_chinese_prompt(self):
        cli = MockCLI()
        r = await cli.execute("处理 task_数据库 的连接泄漏")
        assert r.ok
